...
class SnakeGameEnv:
    ...
    def get_state(self):
        ...
        #Collision states:
        snake_pos = self.get_snake()
        collision_x = 0 # No collision
        collision_y = 0 # No collision

        # collision_y: 0 = no collision; 1 = collision UP;
        #                                2 = collision DOWN
        #                                3 = collision UP and DOWN
        if ([snake_pos[0], snake_pos[1]-10] in self.snake_body) or \
                snake_pos[1]-10 < 0:
            collision_y = 1     # collision UP

        if ([snake_pos[0], snake_pos[1]+10] in self.snake_body) or \
                snake_pos[1] + 10 > self.frame_size_y - 10:
            collision_y += 2     # collision Down

        # collision_x: 0 = no collision; 1 = collision RIGHT;
        #                                2 = collision LEFT;
        #                                3 = collision RIGHT and LEFT
        if ([snake_pos[0] + 10, snake_pos[1]] in self.snake_body) or \
                snake_pos[0] + 10 >self.frame_size_x - 10:
            collision_x = 1   # collision RIGHT

        if ([snake_pos[0] - 10, snake_pos[1]] in self.snake_body) or \
                snake_pos[0]-10 < 0 :
            collision_x += 2  # collision LEFT




        attributes = [direction, rel_x, rel_y, collision_x, collision_y]

        #Adding collision as we do with the others attributes creates states that are
        # not possible,since for example the state of collision going RIGHT implies
        # that the snake is going to the right.
        state = attributes[0] + attributes[1] * 4 + attributes[2] * 12 + \
                attributes[3] * 36 + attributes[4] * 144
    ...
    def compute_dist(self):
        return [abs(self.get_snake()[0] - self.get_food()[0]),
                abs(self.get_snake()[1] - self.get_food()[1])]
    ...
    def calculate_reward(self):
        ...
        if self.get_snake() == self.get_food():
            ...
            reward = -10
        # Reward or penalize whether the move got the snake closer to the
        # food or not
    ...
    def update_snake_position(self, action):
        ...
        if self.snake_pos[0] == self.food_pos[0] and self.snake_pos[1] == self.food_pos[
            1]:
            self.score += 10
